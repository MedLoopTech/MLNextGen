...
### CSRF protection on the auth forms — fixed
Earlier revisions used `.DisableAntiforgery()` on the login/register/logout endpoints and documented it as a known pre-production gap. That's closed: each form now renders Blazor's `<AntiforgeryToken />`, and the endpoints bind `IFormCollection` as a handler parameter — which is what opts a minimal-API endpoint into .NET 8's automatic antiforgery validation. Verified against the running app: posting without a token is rejected with 400; the normal login/register/logout flows work with it. The login `returnUrl` is also validated server-side (same-site paths only) so the login page can't be used as an open redirect.

### Portal UI (production pass)
The portal has had a production-grade UI pass, verified end-to-end against the running app rather than just compiled:
- **App shell**: branded design system in `wwwroot/css/app.css` (Bootstrap 5.3 CSS-variable overrides, not a fork), favicon, mobile navbar toggler (the first cut had none — the menu was unreachable below the `lg` breakpoint), active-link highlighting, footer, and an `ErrorBoundary` around the page body so an unhandled exception shows a recoverable message instead of Blazor's raw error bar.
- **Deep links**: an anonymous browser hit on a protected page redirects to `/account/login?returnUrl=…` instead of the blank 401 the API-style identity cookie produces by default (API clients still get plain 401/403 — split by `Accept` header in `Program.cs`). Unknown URLs get a styled 404 page via a re-execute middleware; note the explicit `UseRouting()` placement it depends on, explained in `Program.cs`.
- **Money formatting**: the app pins a culture at startup (`App:Culture`, default `en-US`) — the container's invariant default rendered every `ToString("C")` as `¤`.
- **Double-submit guards**: every mutating action in the interactive pages (accept/reject/counter/cancel, checkout, fulfill, dispute, feedback) refuses re-entry while in flight. On checkout this is load-bearing, not cosmetic: Blazor Server queues a double-click as two sequential events, and while the DB's unique index on `Order.BidId` would reject a second order row, the second gateway charge would already have happened.
- **Consistency**: shared `StatusBadge`/`LoadingIndicator`/`EmptyState` components so status colors, loading states, and empty states can't drift between pages.
